const questions = {
  group1: [
    ...
    "شو الشيء اللي يعتبره جريمة بحق الأكل؟",
    "لو كنت عطر، شو ريحتك؟",
...
function selectChoice(player, choice) {
  const allButtons = document.querySelectorAll(`.choice-btn[data-player="${player}"]`);
  allButtons.forEach(btn => btn.classList.remove('selected'));
  
  const target = document.querySelector(`.choice-btn[data-player="${player}"][data-choice="${choice}"]`);
...
function flipCoin() {
  const flipBtn = document.getElementById('flipBtn');
  const coinAnim = document.getElementById('coinAnimation');
  const resultDiv = document.getElementById('result');
  const coin = document.querySelector('.coin');
  
  flipBtn.style.display = 'none';
  coinAnim.classList.remove('hidden');
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      resultDiv.innerHTML = `
        <div style="font-size: 32px; margin-bottom:10px;">${result === 'تاج' ? '👑' : '⭐'}</div>
        <p>النتيجة: <strong>${result}</strong></p>
        <p>🎉 ${winner} بيبدأ يسأل!</p>
        <button class="flip-btn" style="margin-top:15px;" onclick="showGroups()">اختر المجموعة 🚀</button>
      `;
...
function nextQuestion() {
  ...
  const asker = (currentTurn === 1) ? player1Name : player2Name;
  const answerer = (currentTurn === 1) ? player2Name : player1Name;
  
  document.getElementById("turnPlayer").innerHTML = `🎯 دور <strong>${asker}</strong> يسأل <strong>${answerer}</strong>`;
...
function answerQuestion() {
    if (timerRunning) {
        stopTimer();
        alert("✅ إجابة موفقة! اضغط 'سؤال جديد' للدور التالي.");
    }
}

function skipQuestion() {
    stopTimer();
    // الخصم من اللاعب الذي كان عليه الدور للإجابة (المستقبل للسؤال)
    // لاحظ أن الدور تم تبديله في nextQuestion فنعكس المنطق
    if (currentTurn === 2) player2Points = Math.max(0, player2Points - 1);
    else player1Points = Math.max(0, player1Points - 1);
    
    updatePointsDisplay();
    alert("🚫 تم الامتناع! خصم نقطة.");
...
function checkGameOver() {
    if (player1Points <= 0 || player2Points <= 0 || currentGroup.length === 0) {
        stopTimer();
        let finalMsg = "";
        if (player1Points > player2Points) finalMsg = `🏆 الفائز هو: ${player1Name}`;
        else if (player2Points > player1Points) finalMsg = `🏆 الفائز هو: ${player2Name}`;
        else finalMsg = "🤝 انتهت بالتعادل!";

        document.getElementById("question").innerHTML = `
            <div style="text-align:center;">
                <h2>انتهت اللعبة!</h2>
                <h3 style="color:#ffd700; margin:15px 0;">${finalMsg}</h3>
                <p>النقاط النهائية:</p>
                <p>${player1Name}: ${player1Points} | ${player2Name}: ${player2Points}</p>
                ${prize ? `<p style="margin-top:10px;">🎁 الجائزة: ${prize}</p>` : ''}
            </div>
        `;
        document.querySelector(".buttons-container").innerHTML = `<button class="action-btn reset-btn" onclick="location.reload()">لعبة جديدة ↺</button>`;
        return true;
    }
    return false;
}
...
// Init
let vCount = localStorage.getItem('visitorCount') || Math.floor(Math.random() * 200);
vCount = parseInt(vCount) + 1;
localStorage.setItem('visitorCount', vCount);
document.getElementById('visitorCount').textContent = vCount;
